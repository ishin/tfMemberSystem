...
- (NSArray *)searchGroupBysearchText:(NSString *)searchText{
  ...
  NSArray *groupArray = [[GoGoDB sharedDBInstance] queryAllGroups];
  for (NSDictionary *gInfo in groupArray) {
      
      RCDGroupInfo *group = [[RCDGroupInfo alloc] init];
      group.groupId = [NSString stringWithFormat:@"%d", [[gInfo objectForKey:@"GID"] intValue]];
