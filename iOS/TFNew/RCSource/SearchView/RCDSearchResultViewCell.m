  ...
  if (!(model.otherInformation.length > 0) || !(model.name.length > 0)) {
    ...
    [self.nameLabel attributedText:str byHighlightedText:self.searchString];
      ...
      if(model.searchType == RCDSearchGroup){
          ...
          self.mask.hidden = NO;
          self.mask.backgroundColor = [Utls groupMaskColorWithId:[model.targetId intValue]];
    ...
    if(model.searchType == RCDSearchGroup){
        ...
        [self.headerView setImageWithURL:[NSURL URLWithString:model.portraitUri] placeholderImage:[RCDUtilities imageNamed:@"default_portrait_msg" ofBundle:@"RongCloud.bundle"]];
    }
    else
    {
...
- (void)loadView{
  ...
  [self.contentView addSubview:self.headerView];
    ...
    self.mask = [[UILabel alloc] initWithFrame:self.headerView.bounds];
    [self.headerView addSubview:mask];
    self.mask.layer.cornerRadius = 24;
    self.mask.clipsToBounds = YES;
    self.mask.hidden = YES;
    self.mask.textColor = [UIColor whiteColor];
    // mask.text = showName;
    self.mask.font = [UIFont boldSystemFontOfSize:24];
    self.mask.textAlignment = NSTextAlignmentCenter;
