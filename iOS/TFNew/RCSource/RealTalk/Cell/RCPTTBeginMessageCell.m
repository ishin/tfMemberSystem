...
#define Default_Text  @"我发起了语音对讲"

@implementation RCPTTBeginMessageCell
+ (CGSize)sizeForMessageModel:(RCMessageModel *)model
      withCollectionViewWidth:(CGFloat)collectionViewWidth
         referenceExtraHeight:(CGFloat)extraHeight {
  RCPTTBeginMessage *message = (RCPTTBeginMessage *)model.content;
  CGSize size = [RCPTTBeginMessageCell getBubbleBackgroundViewSize:message];
...
- (void)setDataModel:(RCMessageModel *)model {
  ...
  self.textLabel.text = Default_Text;
  [self setAutoLayout];
}

- (void)setAutoLayout {
  RCPTTBeginMessage *testMessage = (RCPTTBeginMessage *)self.model.content;
...
+ (CGSize)getTextLabelSize:(RCPTTBeginMessage *)message {
  NSString *text = Default_Text;
  if ([text length] > 0) {
    float maxWidth =
