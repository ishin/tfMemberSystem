...
- (id) initWithFrame:(CGRect)frame
{
    if(self = [super initWithFrame:frame])
    {
        ...
        _btnOn.frame = CGRectMake(300-120-xx, top, 120, 40);
        ...
        [_btnOn setTitle:@"拨打" forState:UIControlStateNormal];
        [_btnOn addTarget:self action:@selector(connectAction:) forControlEvents:UIControlEventTouchUpInside];
        
        
        _btnOff = [UIButton buttonWithColor:RGB(0xc6, 0xd8, 0xdc) selColor:nil];
        _btnOff.frame = CGRectMake(xx, top, 120, 40);
        ...
        [_btnOff setTitle:@"取消" forState:UIControlStateNormal];
...
- (void) parseUInfo:(NSDictionary*)res{
    ...
    _message.text = [res objectForKey:@"mobile"];
...
- (void) connectAction:(id)sender{
    ...
    if(delegate_ && [delegate_ respondsToSelector:@selector(didEmergCallButtonDown:)])
    {
        [delegate_ didEmergCallButtonDown:_message.text];
    }
