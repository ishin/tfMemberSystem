...
- (instancetype)initWithFrame:(CGRect)startFrame
  ...
  if (self) {
    ...
    for (NSInteger j = 0; j < 5; j++) {
      UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(j* (starWidth + space), 0, starWidth, startFrame.size.height)];
      
      btn.enabled = YES;
      btn.tag = j + 1;
      [btn addTarget:self action:@selector(starTapBtn:) forControlEvents:UIControlEventTouchUpInside];
      // 上左下右 星星居中
      [btn setImageEdgeInsets:UIEdgeInsetsMake((startFrame.size.height - starWidth)/2, 0, (startFrame.size.height - starWidth)/2, 0)];
      if (j < index) {
