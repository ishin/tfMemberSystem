...
#define BUTTON_HEIGHT (CLIENT_VIEW_HEIGHT - TITLE_PADDING_TOP - TITLE_HEIGHT - BUTTON_PADDING_TOP_BUTTOM - BUTTON_PADDING_TOP_BUTTOM)
...
- (UIButton *)yesButton {
    if (!_yesButton) {
        _yesButton = [[UIButton alloc] initWithFrame:CGRectMake(BUTTON_PADDING_LEFT_RIGHT, BUTTON_PADDING_TOP_BUTTOM + TITLE_PADDING_TOP + TITLE_HEIGHT, BUTTON_WIDTH, BUTTON_HEIGHT)];

        [_yesButton addTarget:self action:@selector(onYesButton:) forControlEvents:UIControlEventTouchDown];
        [_yesButton setBackgroundImage: [RCKitUtility imageNamed:(@"custom_service_evaluation_yes_hover")ofBundle:@"RongCloud.bundle"] forState:UIControlStateSelected];
        [_yesButton setBackgroundImage: [RCKitUtility imageNamed:(@"custom_service_evaluation_yes")ofBundle:@"RongCloud.bundle"] forState:UIControlStateNormal];
        [_yesButton.layer setMasksToBounds:YES];
        [_yesButton.layer setCornerRadius:3.0];
//        [_yesButton.layer setBorderWidth:0.5f];
//        [_yesButton setTitleColor:[UIColor colorWithRed:198.0/255.0 green:198.0/255.0 blue:198.0/255.0 alpha:1.0f] forState:UIControlStateNormal];
    }
    return _yesButton;
}
- (UIButton *)noButton {
    if (!_noButton) {
        _noButton = [[UIButton alloc] initWithFrame:CGRectMake(BUTTON_PADDING_LEFT_RIGHT + BUTTON_WIDTH + BUTTONS_PADDING, BUTTON_PADDING_TOP_BUTTOM + TITLE_PADDING_TOP + TITLE_HEIGHT, BUTTON_WIDTH, BUTTON_HEIGHT)];
        [_noButton addTarget:self action:@selector(onNoButton:) forControlEvents:UIControlEventTouchDown];
        [_noButton setBackgroundImage: [RCKitUtility imageNamed:(@"custom_service_evaluation_no_hover")ofBundle:@"RongCloud.bundle"] forState:UIControlStateSelected];
        [_noButton setBackgroundImage: [RCKitUtility imageNamed:(@"custom_service_evaluation_no")ofBundle:@"RongCloud.bundle"] forState:UIControlStateNormal];
        [_noButton.layer setMasksToBounds:YES];
        [_noButton.layer setCornerRadius:3.0];
//        [_noButton.layer setBorderWidth:0.5f];
//        [_noButton setTitleColor:[UIColor colorWithRed:198.0/255.0 green:198.0/255.0 blue:198.0/255.0 alpha:1.0f] forState:UIControlStateNormal];
    }
    return _noButton;
}
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        ...
        [_titleLabel setText:NSLocalizedStringFromTable(@"Robot_Comment_Title", @"RongCloudKit", nil)];
