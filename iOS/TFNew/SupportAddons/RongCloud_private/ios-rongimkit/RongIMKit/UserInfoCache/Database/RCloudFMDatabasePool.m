...
- (void)pushDatabaseBackInPool:(RCloudFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The RCloudFMDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
