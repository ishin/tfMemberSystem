...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 - `<RCloudFMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.
 ...
 @warning Do not instantiate a single `RCloudFMDatabase` object and use it across multiple threads. Instead, use `<RCloudFMDatabaseQueue>`.
...
/** Create a `RCloudFMDatabase` object.
 ...
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `RCloudFMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `RCloudFMDatabase` connection is closed.
...
/** Execute select statement
 ...
 Executing queries returns an `<RCloudFMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[RCloudFMResultSet next]>`) from one record to the other.
 ...
 @return A `<RCloudFMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using RCloudFMDatabase::storeableDateFormat.
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using FMDB you will not need to interact directly with `RCloudFMStatement`, but rather with `<RCloudFMDatabase>` and `<RCloudFMResultSet>` only.
