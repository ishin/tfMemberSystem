...
- (RCloudFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(RCloudFMDBExecuteStatementsCallbackBlock)block {
    ...
    rc = sqlite3_exec([self sqliteHandle], [sql UTF8String], block ? RCloudFMDBExecuteBulkSQLCallback : nil, (__bridge void *)(block), &errmsg);
...
void RCloudFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv); // -Wmissing-prototypes
...
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (void*)b, &RCloudFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (__bridge void*)b, &RCloudFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
