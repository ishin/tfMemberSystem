...
/** To perform queries and updates on multiple threads, you'll want to use `RCloudFMDatabaseQueue`.
 ...
 Using a single instance of `<RCloudFMDatabase>` from multiple threads at once is a bad idea.  It has always been OK to make a `<RCloudFMDatabase>` object *per thread*.  Just don't share a single instance across threads, and definitely not across multiple threads at the same time.
 ...
 `RCloudFMDatabaseQueue` will run the blocks on a serialized queue (hence the name of the class).  So if you call `RCloudFMDatabaseQueue`'s methods from multiple threads at the same time, they will be executed in the order they are received.  This way queries and updates won't step on each other's toes, and every one is happy.

 ### See also

 - `<RCloudFMDatabase>`

 @warning Do not instantiate a single `<RCloudFMDatabase>` object and use it across multiple threads. Use `RCloudFMDatabaseQueue` instead.
 
 @warning The calls to `RCloudFMDatabaseQueue`'s methods are blocking.  So even though you are passing along blocks, they will **not** be run on another thread.
