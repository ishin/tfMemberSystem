...
-(RCConversationInfo *)getConversationInfo:(RCConversationType)conversationType
    ...
    if (!cacheConversationInfo) {
        //线程同步读取
        RCConversationInfo *dbConversationInfo = [rcUserInfoReadDBHelper selectConversationInfoFromDB:conversationType targetId:targetId];
        if (dbConversationInfo) {
            [self.cache setObject:dbConversationInfo forKey:conversationGUID];
        }
...
-(void)updateConversationInfo:(RCConversationInfo *)conversationInfo
    ...
    if (![cacheConversationInfo isEqual:conversationInfo]) {
        [self.cache setObject:conversationInfo forKey:conversationGUID];
        
        __weak typeof(self) weakSelf = self;
        dispatch_async(rcUserInfoDBQueue, ^{
            [rcUserInfoWriteDBHelper replaceConversationInfoFromDB:conversationInfo conversationType:conversationType targetId:targetId];
            [weakSelf.updateDelegate onConversationInfoUpdate:conversationInfo];
        });
    }
}

-(void)clearConversationInfoNetworkCacheOnly:(RCConversationType)conversationType
                                    targetId:(NSString *)targetId {
    NSString *conversationGUID = [RCConversationInfo getConversationGUID:conversationType targetId:targetId];
    RCConversationInfo *cacheConversationInfo = self.cache[conversationGUID];
    ...
    if (!cacheConversationInfo) {
        __weak typeof(self) weakSelf = self;
        dispatch_async(rcUserInfoDBQueue, ^{
            RCConversationInfo *dbConversationInfo = [rcUserInfoWriteDBHelper selectConversationInfoFromDB:conversationType targetId:targetId];
...
-(void)clearConversationInfo:(RCConversationType)conversationType
                    targetId:(NSString *)targetId {
    NSString *conversationGUID = [RCConversationInfo getConversationGUID:conversationType targetId:targetId];
    RCConversationInfo *cacheConversationInfo = self.cache[conversationGUID];
...
-(void)deleteImageCache:(RCConversationInfo *)conversationInfo {
    if ([conversationInfo.portraitUri length] > 0) {
        [[RCloudImageLoader sharedImageLoader] clearCacheForURL:[NSURL URLWithString:conversationInfo.portraitUri]];
    }
}
