...
- (instancetype)init:(RCConversationModelType)conversationModelType exntend:(NSObject *)extend {
    self = [super init];
    if (self) {
...
- (instancetype)init:(RCConversationModelType)conversationModelType
    ...
    if (self) {
        ...
        self.hasUnreadMentioned = conversation.hasUnreadMentioned;
        ...
        if (conversation && conversation.draft.length > 0) {
          ...
          NSData *draftData =
              [conversation.draft dataUsingEncoding:NSUTF8StringEncoding];
          if (!draftData) {
            self.draft = conversation.draft;
          } else {
            NSDictionary *draftDict =
                [NSJSONSerialization JSONObjectWithData:draftData
                                                options:kNilOptions
                                                  error:&error];
            if (error) {
              self.draft = conversation.draft;
            } else {
              if ([draftDict isKindOfClass:[NSDictionary class]] && [draftDict.allKeys containsObject:@"draftContent"]) {
...
- (void)updateWithMessage:(RCMessage *)message {
  ...
  if (message.messageDirection == MessageDirection_RECEIVE &&
    ...
    if ([[message.content class] persistentFlag] & MessagePersistent_ISPERSISTED) {
      self.hasUnreadMentioned |= message.content.mentionedInfo.isMentionedMe;
    }
  }
}
...
- (BOOL)isMatching:(RCConversationType)conversationType
  ...
  if (self.conversationModelType == RC_CONVERSATION_MODEL_TYPE_COLLECTION && self.conversationType == conversationType) {
    return YES;
