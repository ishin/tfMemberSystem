...
- (id<SDWebImageOperation>)downloadImageWithURL:(NSURL *)url options:(SDWebImageDownloaderOptions)options progress:(void (^)(NSUInteger, long long))progressBlock completed:(void (^)(UIImage *, NSData *, NSError *, BOOL))completedBlock
{
    ...
    [self addProgressCallback:progressBlock andCompletedBlock:completedBlock forURL:url createCallback:^
    {
        ...
        NSString *specialMd5 = @"?nuage_special_md5=";
        NSRange range = [strUrl rangeOfString:specialMd5];
        NSURL *newUrl = url;
        if(range.location != NSNotFound)
        {
            strUrl = [strUrl substringToIndex:range.location];
            newUrl = [NSURL URLWithString:strUrl];
        }
        ////
        
        
        NSMutableURLRequest *request = [NSMutableURLRequest.alloc initWithURL:newUrl cachePolicy:(options & SDWebImageDownloaderUseNSURLCache ? NSURLRequestUseProtocolCachePolicy : NSURLRequestReloadIgnoringLocalCacheData) timeoutInterval:15];
