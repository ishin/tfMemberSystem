...
public class GroupDaoImpl extends BaseDao<TGroup, Long> implements GroupDao {
	...
	public int createGroup(int userId, String code, String groupname,
		...
		try {
			...
			int volume = StringUtils.getInstance().strToInt(
					PropertiesUtils.getStringByKey("group.volume"));
			int space = StringUtils.getInstance().strToInt(
					PropertiesUtils.getStringByKey("group.space"));
			int spaceUse = StringUtils.getInstance().strToInt(
					PropertiesUtils.getStringByKey("group.spaceuse"));
			int annexLong = StringUtils.getInstance().strToInt(
					PropertiesUtils.getStringByKey("group.annexlong"));
	...
	public List<Object[]> getGroupListWithCreaterInfo(String groupIds) {
		try {
			String hql = (new StringBuilder(
					"select M.id MID,M.account,M.fullname,M.logo,M.telephone,M.email,M.address,M.token,M.sex,M.birthday,M.workno,M.mobile,M.groupmax,M.groupuse,M.intro,G.id GID,G.code,G.name,G.createdate,G.volume,G.volumeuse,G.space,G.spaceuse,G.annexlong,G.notice from t_member M right join t_group G on G.creator_id=M.id where G.id in ("))
					.append(groupIds).append(")").toString();
	...
	public int transferGroup(int userIdInt, int groupIdInt) {
		try {
			String hql = "update TGroup t set t.creatorId=" + userIdInt
					+ " where t.id=" + groupIdInt;
	...
	public int changeGroupName(int groupIdInt, String groupName) {
		try {
			String hql = "update TGroup t set t.name='" + groupName
					+ "' where t.id=" + groupIdInt;
	...
	public int updateGroupMemberNum(int groupId, int i) {
		try {
			String hql = "update TGroup t set t.volumeuse=volumeuse+" + i
					+ " where t.id=" + groupId;
	...
	public Object[] groupInfo(int id) {
		 ...
		 * try {
		 * 
		 * Criteria ctr = getCriteria(); ctr.add(Restrictions.eq("id", id));
		 * 
		 * List<TGroup> list = ctr.list();
		 * 
		...
		try {
			String hql = (new StringBuilder(
					"select M.id MID,M.account,M.fullname,M.logo,M.telephone,M.email,M.address,M.token,M.sex,M.birthday,M.workno,M.mobile,M.groupmax,M.groupuse,M.intro,G.id GID,G.code,G.name,G.createdate,G.volume,G.volumeuse,G.space,G.spaceuse,G.annexlong,G.notice from t_member M right join t_group G on G.creator_id=M.id where G.id="))
					.append(id).toString();
			SQLQuery query = getSession().createSQLQuery(hql);
			List list = query.list();
			if (list.size() > 0)
