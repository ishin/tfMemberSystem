...
var membertemplate = '<div id="mmemberid" name="membername" class="member21">'
						...
						+ '<img src="page/admin/images/delete-2.png" /></div>'
						+ '</div>';
...
//	$('.line21').css('width', document.body.clientWidth * 0.12 + 'px');
	...
	$('body').on('click', '#list21 li', function() {
		...
		$(this).after('<img id="sanjiao" src="page/admin/images/roleselect.png" style="float:right" />');
		if (curpage == '212') {
			showpage('210');
		}
	...
	$('body').on('click', '.privgroup, .privgroupd', function() {
		if ($(this).prop('src').indexOf('1.png') > 0) {
			$(this).parent().parent().find('img').prop('src', 'page/admin/images/select-2.png');
			$(this).parent().parent().find('input').prop('checked', false);
		}
		else {
			$(this).parent().parent().find('img').prop('src', 'page/admin/images/select-1.png');
			$(this).parent().parent().find('input').prop('checked', true);
		}
	});
	$('body').on('click', '.pgc, .pgcd', function() {
		if ($(this).prop('src').indexOf('1.png') > 0) {
			$(this).parent().find('img').prop('src', 'page/admin/images/select-2.png');
			$(this).parent().find('input').prop('checked', false);
		}
		else {
			$(this).parent().find('img').prop('src', 'page/admin/images/select-1.png');
			...
			while (j--) {
				if (data[j].parentid == data[i].privid) {
					...
					var g = '<div class="line2111d">'
						+ '<img src="page/admin/images/select-2.png" class="privgroupd pgcgd" />'
					...
					while (k--) {
						if (data[k].parentid == data[j].privid) {
							var gp = '<div class="priv2d toleft">'
								+ '<img src="page/admin/images/select-2.png" class="pgcd" />'
...
function cb_210_fresh(data) {
	...
	while(i--) {
		...
		$('#list210 tr:last-child')
			...
			.append('<td><img src="page/admin/images/delete-2.png" onclick="del210(' + data[i].memberroleid + ')"></img></td>');
	}
			...
			while (j--) {
				if (data[j].parentid == data[i].privid) {
					...
					var g = '<div class="line2111">'
						+ '<img src="page/admin/images/select-2.png" class="privgroup pgcg">'
						...
						if (data[k].parentid == data[j].privid) {
							...
							if (data[k].roleid == currole) {
								gp = '<div class="priv2 toleft">'
									+ '<img src="page/admin/images/select-1.png" class="pgc">'
									+ '<input type="checkbox" id="p' + data[k].privid + '" style="display:none" checked />' 
									+ data[k].privname + '</div>';
							}
							else {
								gp = '<div class="priv2 toleft">'
									+ '<img src="page/admin/images/select-2.png" class="pgc">'
...
function cb_21_fresh(data) {
	...
	$('#list21').find('li:first-child').after('<img id="sanjiao" src="page/admin/images/roleselect.png" style="float:right" />');
	load210();
	load211();
	load212();
	load213();
...
function cb_21_del(data) {
	...
	$b.after('<img id="sanjiao" src="page/admin/images/roleselect.png" style="float:right" />');
