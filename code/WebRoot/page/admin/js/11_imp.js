...
$(document).ready(function() {
	...
	$('.result').click(function() {
		...
		if ($(img).prop('src').indexOf('open') > 0) {
			$(img).prop('src','page/admin/images/close.png');
			$(this).parent().next().hide();
		}
		else {
			$('#content').find('.result').find('img').prop('src', 'page/admin/images/close.png');
			$('#content').find('.resultlist').hide();
			$(img).prop('src', 'page/admin/images/open.png');
...
function showlist(result) {
	...
	$(resultlist).prev().find('img').prop('src', 'page/admin/images/open.png');
	$(resultlist).show();
}
function leave(a) {
	var text = $(a).val();
...
function edittr(tr, data, group) {
	...
	$(ntr).append('<td><img class="deltr" src="page/admin/images/delete.png" title="删除" /></td>');
	$(tr).remove();
}
function addtr(tb, data, group) {
	...
	$(tr).append('<td><img class="deltr" src="page/admin/images/delete.png" title="删除" /></td>');
}
