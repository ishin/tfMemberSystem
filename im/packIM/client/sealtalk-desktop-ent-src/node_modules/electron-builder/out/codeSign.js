...
function createKeychain(keychainName, cscLink, cscKeyPassword, csaLink) {
    const authorityCertPath = path.join(os_1.tmpdir(), randomString() + ".cer");
    const developerCertPath = path.join(os_1.tmpdir(), randomString() + ".p12");
    const keychainPassword = randomString();
    return promise_1.executeFinally(bluebird_1.Promise.all([httpRequest_1.download(csaLink || "https://developer.apple.com/certificationauthority/AppleWWDRCA.cer", authorityCertPath), httpRequest_1.download(cscLink, developerCertPath), bluebird_1.Promise.mapSeries([["create-keychain", "-p", keychainPassword, keychainName], ["unlock-keychain", "-p", keychainPassword, keychainName], ["set-keychain-settings", "-t", "3600", "-u", keychainName]], it => util_1.exec("security", it))]).then(() => importCerts(keychainName, authorityCertPath, developerCertPath, cscKeyPassword)), errorOccurred => {
        const tasks = [fs_extra_p_1.deleteFile(authorityCertPath, true), fs_extra_p_1.deleteFile(developerCertPath, true)];
        if (errorOccurred) {
...
function importCerts(keychainName, authorityCertPath, developerCertPath, cscKeyPassword) {
    return __awaiter(this, void 0, void 0, function* () {
        yield util_1.exec("security", ["import", authorityCertPath, "-k", keychainName, "-T", "/usr/bin/codesign"]);
