        ...
        operations.push(function (cb) {
          common.normalizeExt(opts.icon, '.ico', function (err, icon) {
            ...
            if (!err) {
              ...
              function exercedit(callback)
              {
                  require('rcedit')(path.join(tempPath, newExeName), rcOpts, function (err) {
                      console.log('['+err+']');
                      if(err=='Error: rcedit.exe failed with exit code 1. Fatal error: Unable to commit changes' && retryCount)
                      {
                          retryCount --;
                          console.log('[rcedit.exe] error!retry...',retryCount);
                          exercedit(callback);
                      }
                      else
                      {
                          callback(updateWineMissingException(err));
                      }
                  })
              }
              exercedit(cb);
